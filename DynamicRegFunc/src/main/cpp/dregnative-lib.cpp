...
#endif
//定义类名
static const char *className = "com/example/drfunc/DynamicRegistrJni";

//定义对应Java native方法的 C++ 函数，函数名可以随意命名
static jstring sayHello(JNIEnv *env, jobject) {
    LOGE("Native hello, this is native log.");
...
static int registerNativeMethods(JNIEnv *env, const char *className,
    ...
    jclass clazz;
    LOGE("Native Registering %s natives\n", className);
    ...
    if ((env)->RegisterNatives(clazz, gMethods, numMethods) < 0) {
        LOGE("Native Register natives failed for '%s'\n", className);
        return JNI_ERR;
    }
    //删除本地引用
...
JNIEXPORT void JNICALL Java_com_example_drfunc_DynamicRegistrJni_registerNatives
        (JNIEnv *env, jclass clazz){
    LOGE("Native call registerNatives");
